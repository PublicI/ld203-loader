...
for sourceFile in $needUpdates; do
	...
	wget -N http://soprweb.senate.gov/downloads/$sourceFile -O downloads/$sourceFile

    echo "Extracting: $sourceFile"
    unzip downloads/$sourceFile -d extracted/${sourceFile%%.*}
...
csvstack --filenames ./parsed/**/*_contributions.csv > dist/all_contributions.csv
echo "Merging $(ls ./parsed/**/*_meta.csv | wc -l) metadata files into all_meta.csv"
csvstack --filenames ./parsed/**/*_meta.csv > dist/all_meta.csv
