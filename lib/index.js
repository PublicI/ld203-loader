...
function getDirectories(srcPath) {
    ...
    let directories = allFiles.filter(file => fs.lstatSync(path.join(srcPath, file)).isDirectory())
    return directories;
}
...
async function download(yargs) {
    ...
    .mergeWithLimit(1)
    .done(function () {
        console.log(`Finished downloading: ${toDownload.availableFiles.length} files.`);
    });
}
...
function convertAll(yargs) {
    ...
    .map(f => path.resolve(inputDir, f))
    .map(f => _(convert({chamber: yargs.chamber, inputXML: f, outputDir: outputDir})))
    .mergeWithLimit(1)
}
...
async function mergeFiles(yargs){
    ...
    .map(d => {
        ...
        .sequence().map(f => path.resolve(d, f))
    })
    .sequence()
...
.command('batchConvert <chamber> [options]', 'batch convert all the downloads', (yargs) => {
    return yargs.demandOption(['inputDir', 'outputDir'])
}, batchConvert)
.command('mergeFiles <chamber> <type> [options]', 'merge json files into single output file', (yargs) => {
