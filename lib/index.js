...
async function batchConvert(yargs) {
    ...
    directories.forEach(async (f) => {
        let results = await convertAll({
            chamber: yargs.chamber,
            inputDir: path.resolve(inputDir, f),
            outputDir: yargs.outputDir
        })
        return results
    })
    // Search the input directory for folders and pass in these arguments into the covertAll() function above.
}
