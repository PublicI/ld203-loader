...
const senate_url =
    'https://www.senate.gov/legislative/Public_Disclosure/contributions_download.htm';
...
function fetch() {
    return new Promise((resolve, reject) => {
        x(senate_url, '#secondary_col2 > table table > tr', [
            {
                year:
                    'td:nth-of-type(2) > a@href | resolveFileName | resolveYear',
            ...
            if (err) reject(err);
                ...
                data = data.filter(d => d.hasOwnProperty('link'));
                resolve({ availableFiles: data });
            }
        });
    });
}
...
async function needUpdates(data) {
    ...
    let updates = data.filter(
        d =>
            prevState[d.fileName]
                ? prevState[d.fileName].updated < d.updated
                : true
    );
    return updates;
}
...
async function mapFilingMeta(filing, source_file) {
    let filingMeta = {};
    Object.keys(filing)
        .filter(d => !['Contributions', 'xmlns'].includes(d))
        .forEach(d => {
            if (typeof filing[d] != 'string') {
                Object.keys(filing[d]).forEach(key => {
                    if (key != 'xmlns')
                        filingMeta[`${d}${key.replace(d, '')}`] = filing[d][
                            key
                        ].replace(/(\r\n|\n|\r)/gm, ' ');
                });
            } else {
...
async function mapFilingContributions(filing, source_file) {
    ...
    if (filing.hasOwnProperty('Contributions')) {
        ...
        let contributions = Array.isArray(rawContributions)
            ? rawContributions
            : [rawContributions];
