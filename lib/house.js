...
const house_url =
    'http://disclosures.house.gov/lc/LCDownload.aspx?KeepThis=true&';
...
const x = Xray({
    filters: {
        ...
        resolveUpdated: value =>
            value ? Date.parse(value.split(/[()]/)[1].trim()) : null,
        //Parse date string
...
async function mapFilingMeta(filing, source_file) {
    let filingMeta = {};
    Object.keys(filing)
        .filter(d => !['contributions', 'pacs', 'xmlns'].includes(d))
        .forEach(d => {
    ...
    filingMeta['comments'] = filingMeta['comments']
        ? filingMeta['comments'].trim()
        : '';
